...
# --- NAYI WALI API KEY YAHAN HAI ---
GEMINI_KEY = "AIzaSyBvkE49kLx0pURGIaMX0HvIbQL5kMkGlWM"

try:
    genai.configure(api_key=GEMINI_KEY)
    model = genai.GenerativeModel('gemini-1.5-flash') # Maine model thoda fast wala kar diya hai
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Helvetica Neue', sans-serif; }
        ...
        .result-area { margin-top: 25px; text-align: left; background: #fafafa; padding: 15px; border-radius: 12px; border: 1px solid #eee; white-space: pre-wrap; color: #333; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <form method="POST">
            <textarea name="topic" placeholder="Example: Jaipur trip..." required></textarea>
            <button type="submit" class="gen-btn">Generate Magic ✨</button>
        </form>
        {% if results %}<div class="result-area">{{ results }}</div>{% endif %}
...
def home():
    ...
    if request.method == "POST":
        ...
        try:
            # Yahan humne prompt ko thoda simple kar diya hai
            response = model.generate_content(f"Write 3 short Instagram captions for: {topic}")
            results = response.text
        except Exception as e:
            error = f"AI thoda thak gaya hai: {str(e)}"
